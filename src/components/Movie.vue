<template>
  <div id="movie" :style="{ backgroundImage: `url(${ image_path }${ movie.backdrop_path })` }">
        ...
        <div id="trailer" :class="{ 'novideo' : !videoId}">
            <iframe v-if="videoId" width="800" height="480" :src="videoURL" 
...
export default {
  ...
  computed:{
     videoURL: function(){
       return this.videoLink + this.videoId + "?&autoplay=1";
     }
  },
  mounted: function(){
   let  id = this.movieID;
        axios.get(`https://api.themoviedb.org/3/movie/${ id }?api_key=${requests.api_key}&language=en-US`)
                .then((res) => {
                  ...
                  this.getData();
                    ...
                    }else if(this.movie.original_title){
                          name = this.movie.original_title
                    }
                    movieTrailer(name , {id: true, multi: true} )
                              .then( response => this.videoId = response[0] )
                })
                .catch(err =>{
                  console.log(err);
                })
